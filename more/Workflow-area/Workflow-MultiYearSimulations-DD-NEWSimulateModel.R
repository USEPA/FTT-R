...
parameters[[currentScenarioName]]<-TemplateToParameters(chosenLifeHistoryParameters,daysToRep=3*365)
...
## Run the model
tempOutputs<-lapply(parameters[inputScenariosToRun],SimulateModel,n_0=inputNumberInitInd,dates=1:(365*3))
## End run the model
...
modelOutputListTest$`Y1.Baseline 1`$dailySummary

modelOutputListTest<-unlist(modelRuns,recursive=F)[inputScenariosForResults]
View(modelOutputListTest)
...
FishToxTranslator:::PlotPopulation(modelOutputListTest)
...
test<-unlist(modelRuns,recursive=F)[inputScenariosForResults]

test[[1]]$dailySummary$population
...
#Set up parallel model runs using package "parallel"
tempOutputs2<-lapply(parameters[inputScenariosToRun],SimulateModel,z_t_0=inputz_t_0_Y4)
